 ...
 * @since 1.0.0
 */
function best_hotel_ocdi_after_import() {
...
add_action( 'pt-ocdi/after_import', 'best_hotel_ocdi_after_import' );
 ...
 */
function best_hotel_additional_intro_text( $intro ) {
	$info = '';

	$info .= '<div class="ocdi__intro-text">';
	$info .= '<p><strong>' . esc_html__( 'NOTE: ', 'best-hotel' );
	$info .= esc_html__( 'Before importing, please make sure that Advanced Booking Calendar plugin is installed and activated. Rooms should be properly setup to display room sections in the front page.', 'best-hotel' );
	$info .= '</strong></p>';
	$info .= '</div>';
...
add_filter( 'pt-ocdi/plugin_intro_text', 'best_hotel_additional_intro_text' );
